...
- Session 1-5에서 사용한 `linux-study` 폴더를 VSCode에서 엽니다.
- 이번 세션은 새 명령어를 많이 외우는 것보다, 팀에서 어떤 규칙으로 협업할지 정리하는 데 목적이 있습니다.
- 실제 리눅스 학습에 들어가기 전에 Git 작업 흐름을 스스로 설명할 수 있는지 확인합니다.
...
```bash
git switch -c study/<아이디>-session6
```

예:

```bash
git switch -c study/hoyoung94-session6
```
...
git push -u origin study/<아이디>-session6
```
...
git pull origin main
git switch -c study/hoyoung94-session6
git add members/hoyoung94/YYYY-MM-DD-git-session6.md
git commit -m "docs: [git-session6] 브랜치 전략과 협업 운영 정리"
git push -u origin study/hoyoung94-session6
```
