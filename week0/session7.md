...
- 기존에 clone한 `linux-study` 폴더 안에서는 `git init`을 실행하지 않습니다.
- `git init` 실습은 `linux-study` 폴더 바깥에 별도 연습 폴더를 만들어 진행합니다.
- 실제 제출물은 기존처럼 `linux-study` 저장소의 `members/` 폴더에 작성합니다.
...
```bash
git switch -c study/<아이디>-session7
```

예시:

```bash
git switch -c study/hoyoung94-session7
```

### 4-3. 별도 연습 폴더에서 `git init` 실행하기

`git init`은 이미 Git 저장소인 `linux-study` 안에서 실행하지 않습니다.  
아래처럼 `linux-study`의 상위 폴더로 나가서 별도 연습 폴더를 만듭니다.
...
### 4-8. `linux-study`로 돌아와 제출 문서 만들기

연습 폴더에서 실습한 내용은 별도 Git 저장소에 남겨둡니다.  
제출 문서는 다시 `linux-study` 저장소에 작성합니다.
...
여기서부터는 별도 연습 폴더가 아니라 `linux-study` 저장소에서 실행합니다.
...
git push -u origin study/<아이디>-session7
```
...
- `linux-study`로 돌아와 제출 문서를 만들었다.
- `study/<아이디>-session7` 브랜치에서 작업했다.
