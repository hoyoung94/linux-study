...
이 문서는 `linux-study` 저장소에서 GitHub 협업 흐름을 연습하기 위한 문서입니다.

- Session 1-3에서 사용한 `linux-study` 폴더를 VSCode에서 엽니다.
...
스터디 저장소에서는 `study/<아이디>-session4` 형식을 사용합니다.  
...
```bash
git switch -c study/<아이디>-session4
```

예:

```bash
git switch -c study/hoyoung94-session4
```
...
git add members/<본인GitHub아이디>/YYYY-MM-DD-git-session4.md
git commit -m "docs: [git-session4] GitHub Flow와 PR 리뷰 워크플로우"
git push -u origin study/<아이디>-session4
```
...
git pull origin main
git switch -c study/hoyoung94-session4
git add members/hoyoung94/YYYY-MM-DD-git-session4.md
git commit -m "docs: [git-session4] GitHub Flow와 PR 리뷰 워크플로우"
git push -u origin study/hoyoung94-session4
```
