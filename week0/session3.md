...
- Session 2에서 사용한 `linux-study` 폴더가 내 컴퓨터에 있어야 합니다.
- 이번 세션에서도 `main`을 최신화한 뒤 새 브랜치에서 작업합니다.
- 충돌은 반드시 전용 실습 파일인 `week0/conflict-practice/rolling-paper.md`에서만 만듭니다.
...
```bash
git switch -c study/<아이디>-session3
```

예:

```bash
git switch -c study/hoyoung94-session3
```

`git switch`가 안 되면 아래 명령어를 사용해도 됩니다.

```bash
git checkout -b study/<아이디>-session3
```
...
```text
week0/conflict-practice/rolling-paper.md
```
...
```text
week0/conflict-practice/rolling-paper-group-a.md
week0/conflict-practice/rolling-paper-group-b.md
```
...
git switch main
git pull origin main
git switch study/<아이디>-session3
git merge main
```

충돌이 나면 `week0/conflict-practice/rolling-paper.md` 파일에 conflict marker가 생깁니다.  
...
git status
git add week0/conflict-practice/rolling-paper.md
git add members/hoyoung94/YYYY-MM-DD-git-session3.md
git commit -m "docs: [git-session3] 충돌과 merge 해결 실습"
git push -u origin study/hoyoung94-session3
git switch main
git pull origin main
git switch study/hoyoung94-session3
git merge main
git add week0/conflict-practice/rolling-paper.md
git commit
git push
...
- `study/<아이디>-session3` 브랜치를 만들었다.
- `week0/conflict-practice/rolling-paper.md`의 같은 줄을 내 메시지로 수정했다.
