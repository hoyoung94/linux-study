...
이 문서는 `linux-study` 저장소에서 안전하게 복구 개념을 익히기 위한 문서입니다.
...
```bash
git switch -c study/<아이디>-session5
```

예:

```bash
git switch -c study/hoyoung94-session5
```
...
git push -u origin study/<아이디>-session5
```
...
git stash list
git stash pop
git add members/hoyoung94/YYYY-MM-DD-git-session5.md
git commit -m "docs: [git-session5] 실수 복구와 stash 실습"
git push -u origin study/hoyoung94-session5
```
