...
이 문서에서 말하는 `linux-study`는 **각자 컴퓨터에 `git clone`으로 내려받은 저장소 폴더**입니다.

- 새 빈 폴더를 임의로 만드는 것이 아닙니다.
- 경로는 사람마다 다를 수 있습니다.
- 중요한 것은 **clone으로 생성된 `linux-study` 폴더를 VSCode에서 여는 것**입니다.
...
3. 생성된 `linux-study` 폴더를 VSCode에서 엽니다.
...
그다음 생성된 `linux-study` 폴더를 VSCode로 엽니다.
...
```bash
git checkout -b study/<아이디>-session1
```

확인할 것:
- 현재 브랜치가 `study/<아이디>-session1`로 바뀌었는지
...
확인할 것:
- 제출 문서 파일명이 `2026-04-24-git-session1.md`인지
