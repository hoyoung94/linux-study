...
## 🛠️ 1. 실습 환경 준비 {toggle="true"}

아래 세 가지 중 본인 환경에 맞는 방법을 선택하세요.

#### 1-1. Windows 사용자 — WSL2 (권장) {toggle="true"}
...
> 💡 **팁**<br>Windows 11에서는 Microsoft Store에서 "Ubuntu"를 검색해 직접 설치도 가능해요.

#### 1-2. Mac 사용자 — 기본 터미널 {toggle="true"}

macOS는 Unix 기반이라 별도 설치 없이 **기본 터미널**을 그대로 사용하면 돼요.

- `Cmd + Space` → "터미널" 검색 → 실행

#### 1-3. VMware + Ubuntu Server 24.04 설치 {toggle="true"}

**\*\* vmware 설치되있으신 분들은 1-4부터 하시면 됩니다!** {color="red"}
...
> 💡 **참고**<br>Mac 사용자는 VMware Fusion을 다운로드하세요. (M1/M2 Mac은 UTM 또는 Parallels 권장)

### 1-3. Ubuntu Server 24.04 ISO 다운로드
...
> ⚠️ **주의**<br>Desktop이 아닌 **Server** 버전을 받으세요. 페이지 잘 확인!
...
9. **Profile setup** ⭐ 중요
   - Your name: 본인 이름
   - Server name: `ubuntu-server` (호스트명)
...
> 💡 **재부팅 시 ISO 제거 메시지**<br>"Please remove installation media" 메시지가 뜨면 Enter를 누르세요. VMware가 알아서 제거해요.
...
## 🛜 2. MobaXterm으로 SSH 접속 {toggle="true"}
...
<tr>
<td>Remote host</td>
<td>가상머신 IP (예: 192.168.xxx.xxx)</td>
</tr>
<tr>
<td>Specify username</td>
<td>✅ 체크 후 `ubuntu` 입력</td>
</tr>
<tr>
<td>Port</td>
<td>`22` (기본값)</td>
...
> 💡 **비밀번호 저장 (선택)**<br>MobaXterm이 비밀번호를 기억하게 할 수 있어요. 다만 보안을 위해 마스터 패스워드 설정을 권장해요.<br>Settings → Configuration → General → Master password

---

## 🖥️ 3. 터미널 첫 명령어 {toggle="true"}
...
> ⚠️ **위험 경고**`rm -rf /` 는 **시스템 전체 삭제**예요. `rm -rf` 를 쓸 때는 경로를 두 번 확인하세요. **휴지통이 없어요!**
...
> 💡 **실무 꿀팁 — tail -f**<br>배포 후 로그를 실시간으로 보는 가장 자주 쓰는 명령어예요. 에러가 발생하는 순간을 바로 확인할 수 있어요.<br>종료는 `Ctrl + C`
...
3. `tail -f` 로 시스템 로그를 1분간 모니터링해보고, 어떤 메시지가 나오는지 확인해보세요.
...
현재 상태를 저장하고 복원할 수 있어요. **실수해도 되돌릴 수 있어요.**

**스냅샷 찍기**

1. VMware 메뉴 → **VM** → **Snapshot** → **Take Snapshot**
2. 이름 입력 (예: `Session1 완료`)

**복원**

- VM → Snapshot → Snapshot Manager → 원하는 시점 선택 → Go To

> 💡 **추천 스냅샷 시점**
>
> 1. Ubuntu 설치 + 초기 설정 직후
> 2. SSH 접속 성공 직후
> 3. 각 세션 시작 전
